...
public class Account {
	...
	public void setRegistrationInfo(RegistrationRequest request, Boolean savePageInfo) {
		...
		if (request.getTrackingProviders() == null)
			request.setTrackingProviders(new ArrayList<String>());
		this.setIndustries(request.getIndustry());
		this.setDataTrackingProviders(request.getTrackingProviders());
	...
	public Account(Long id, String siteUrl, AccountStatus status, String username, String email,
			BusinessInformation businessInformation, Integer sessionTimeout, OffsetDateTime createdOn,
			OffsetDateTime modifiedOn, List<String> industries, List<String> dataTrackingProviders, List<Page> pages) {
		super();
		this.id = id;
		...
		this.dataTrackingProviders = dataTrackingProviders;
		this.pages = pages;
	}
